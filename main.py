...
def verify_transaction(tx_hash, expected_amount):
    try:
        ...
        for tx in data.get("result", []):
            ...
            if tx_hash_found == tx_hash and abs(amount - expected_amount) < 0.01:  # تحمل خطای کوچک
...
def button(update, context):
    ...
    try:
        if query.data == "check_membership":
            ...
            if "waiting_for_tx_hash" in context.user_data:
...
def handle_message(update, context):
    ...
    if "waiting_for_wallet" in context.user_data and context.user_data["waiting_for_wallet"]:
        ...
        if verify_transaction(tx_hash, amount):
            ...
            # کاربر می‌تواند دوباره TX Hash بفرستد، پس waiting_for_tx_hash حذف نمی‌شود
    else:
