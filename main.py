...
import random
import logging
from tonclient.client import TonClient as TONClient  # تغییر به ton-client-py
from tonclient.types import ParamsOfQueryCollection  # برای کوئری تراکنش‌ها
...
# اتصال به TON
ton_client = TONClient(network={'server_address': TON_API_URL, 'api_key': TON_API_KEY})
...
# بررسی تراکنش با سرور TON
def verify_transaction(tx_hash, expected_amount):
    try:
        # استفاده از API JSON-RPC به جای متد مستقیم
        params = ParamsOfQueryCollection(
            collection="messages",
            filter={
                "msg_type": {"eq": 0},  # پیام ورودی
                "dst": {"eq": WALLET_ADDRESS},
                "id": {"eq": tx_hash}
            },
            result="value"
        )
        result = ton_client.query_collection(params)
        if result and result[0]["value"] == str(expected_amount * 10**9):  # تبدیل به نانوTON
            return True
        return False
...
def handle_message(update, context):
    ...
    if "waiting_for_wallet" in context.user_data and context.user_data["waiting_for_wallet"]:
        ...
        if verify_transaction(tx_hash, amount):
            ...
            update.message.reply_text(
                "⚠️ Transaction not verified yet. Please wait or check your TX Hash.",
                reply_markup=main_menu()
            )
            bot.send_message(
                ADMIN_ID,
