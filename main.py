...
def check_membership(bot, user_id):
    result = all(is_member(bot, user_id, chat) for chat in REQUIRED_CHATS)
    logger.info(f"Full membership check for {user_id}: {result}")
    return result
...
def button(update, context):
    ...
    try:
        ...
        if query.data == "check_membership":
            missing_chats = get_missing_chats(bot, user_id)  # دوباره چک کن
            if not missing_chats:
